...
import { getCommitments, getPaymentRefs, createCommitment, updateCommitmentStatus } from '../api/client'
...
export default function Commitments() {
  ...
  function openForm() {
    setError(null); setReference(''); setAmount('')
    setDate(new Date().toISOString().slice(0, 10)); setCounterparty(''); setStatus('active')
  ...
  async function handleCreate(e: React.FormEvent) {
    ...
    try {
      await createCommitment({
        ...
        counterparty: counterparty || undefined, status,
      })
            ...
            <table>
              ...
              <tbody>
                {loading ? (
                  ...
                  <tr><td colSpan={7} className="empty-state">No commitments for {year}.</td></tr>
                ) : items.map(c => (
                  <tr key={c.id}>
